...
3. **Body — required structure:**
   ...
   - **Never write Codex's `@`-handle in the body** — not even quoted in backticks, and not when the PR is *about* the review loop. The connector matches the raw PR description, so a quoted handle reads as a task and Codex opens the PR in **action mode**: it writes code and claims a follow-up PR instead of reviewing, and the commit never lands. Write "the bare Codex review trigger" instead. (Confirmed on intelligence-core#63 — the same branch got a normal review once the handle came out of the description.) Same rule for issue bodies and PR comments; see the `codex-review-triage` skill.
