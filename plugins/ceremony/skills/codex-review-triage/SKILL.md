...
containing the literal string `@codex review` — that exact phrase is the only
trigger Codex's bot documents. Do not nudge while CI is still running.
...
— it spawns a fresh session to post `@codex review` and removes the label.
...
when CI goes green, you may post `@codex review` yourself and skip the label
dance — but still bump the round counter so the hard-stop stays accurate.
...
yourself — see the `/delegate-to-codex` command. Use the `codex-task` /
